...
function wp_cache_append_tag( &$buffer ) {
    ...
    //If response content is JSON, do not add html contents...
    if (wp_cache_is_json($buffer)) {
        return false;
    }
...
/*
* Auxiliary function to check if output is JSON
*
*/
function wp_cache_is_json(&$buffer) {
    json_decode($buffer);
...
function wp_cache_get_ob(&$buffer) {
    ...
    if ( !preg_match( apply_filters( 'wp_cache_eof_tags', '/(<\/html>|<\/rss>|<\/feed>|<\/urlset|<\?xml)/i' ), $buffer ) && !wp_cache_is_json($buffer) ) {
		$new_cache = false;
