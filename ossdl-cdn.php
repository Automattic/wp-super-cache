...
if ( false == get_option( 'ossdl_off_cdn_url' ) ) {
    add_option( 'ossdl_off_cdn_url', get_option( 'siteurl' ) );
}
if ( false == get_option( 'ossdl_off_blog_url' ) ) {
...
if ( false == get_option( 'ossdl_cname' ) ) {
 ...
 *
 * @return bool true if to exclude given match from rewriting
 */
function scossdl_off_exclude_match($match, $excludes)
{
    foreach ( $excludes as $badword ) {
        if ( !empty( $badword ) && false !== stripos( $match, $badword ) ) {
            return true;
        }
    }
...
function scossdl_off_rewriter( $match )
{
    ...
    if ( $ossdl_https && 'https' == substr( $match[0], 0, 5 ) ) {
        return $match[0];
    }
...
function scossdl_off_update()
{
    ...
    if ( $valid_nonce && isset( $_POST[ 'action' ] ) && ( 'update_ossdl_off' == $_POST[ 'action' ] ) ) {
